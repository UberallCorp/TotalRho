controllerModule.controller('seleccionClienteController', function ($scope, $rootScope, $state, $ionicLoading, ClientesFactory, EntregaFactory){
	//No Menu items for this.
	//$rootScope.menuItems = [];
	//ClientesFactory.loadAll();
	//$scope.clientes = ClientesFactory.clientes;

	$scope.clientSeach = "Ingrese Cliente";
	...
	$scope.selectCliente = function(cliente){
		...
		var entregaElement = EntregaFactory.create({
			...
			'PrecioUnitario': cliente.precio,
			'CodigoTipoCliente': cliente.codigo_TipoCliente,
			'Enviado':'0'
		});
		var temp = JSON.stringify(entregaElement);
		$state.go('app.detalleEntrega',{numeroPedido: entregaElement.NumeroPedido});
	}
});